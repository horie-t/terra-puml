...
public class PlantUmlGeneratorTest {
    ...
    @Test
    void test_NLBのリソースが生成される() throws IOException {
        // arrange
        var inputFile = new File(BASE_PATH + "network_lb/input/main.tf");
        var layoutFile = new File(BASE_PATH + "network_lb/input/layout.puml");
        // act
        var actual = sut.generateFromTerraform(inputFile, Optional.of(layoutFile));
        writeToFile("network_lb/actual/main.puml", actual);
        // assert
        assertEquals(Files.readString(Path.of(BASE_PATH + "network_lb/expect/main.puml")), actual);
    }
